...
var OKSDK = (function () {
    ...
    const MOBILE = 'mobile', WEB = 'web', UNKNOWN = 'unknown';
    ...
    var DEPRECATED_SIG_PARAMS = ['sig', 'access_token'];
    var state = window.sdk_state = {
        app_id: 0, app_key: '',
    ...
    function init(args, success, failure) {
        ...
        state.mode = (params['mode'] || hParams['mode'])
            || (params['api_server']
        ...
        if (!params['api_server']) {
            if ((hParams['access_token'] == null) && (hParams['error'] == null)) {
                window.location = state.widgetServer + 'oauth/authorize' +
                    ...
                    '&layout=' + (state.mode || args.oauth.layout || 'a') +
                    '&mode=' + (state.mode || '') +
        ...
        //
        //    var fn = callbackRegistry[e.data.callback];
        //    if (fn) {
        //        fn(e.data);
        //    }
        //}
     ...
     * @constructor
     */
    function ApiRequestConstructor(widget, options) {
        this.state = state;
        this.widget = widget;
    ...
    ApiRequestConstructor.prototype = {
        context: {
        ...
        widgetRegistry: {
            /* widgetName: {uiMethod, rest} */
            'WidgetGroupAppPermissions': null,
            'WidgetMediatopicPost': { uiMethod: 'postMediatopic', rest: null },
            'WidgetInvite': null,
            'WidgetSuggest': null
            ,
        },
        resolveContext: resolveContext,
        runAdapter: function (adapterFn) {
            adapterFn && adapterFn.call(this);
        },
        run: function () {
            /* DEBUG */ window.sdk_context = this.context;
            var isMob = this.context.platform === MOBILE;
            var uiMethod = this.widgetRegistry[this.widget];

            if (this.context.isSelfHosted) {
                console.log('::HOSTED!');
                if (isMob) {
                    console.log('::HOSTED::MOB!');
                    // ok-layer, iframe-layer
                    //createIframe(getLinkOnWidget(this.widget, this.options));
                    ...
                    console.log('::HOSTED::WEB!');
                    // popup
                    widgetOpen(this.widget, this.options);

                    // ok-layer, iframe-layer
                    //OKSDK.invokeUIMethod(uiMethod, this.options, 'on');
                }
            } else if (isMob) {
                console.log('::MOB!');
                // popup
                ...
                console.log('::WEB! -> postMessage');
                //как надо: this.adapters[this.context].prepare();
                this.runAdapter();
    ...
    */
    ...
    //        }
    //    }
    //    console.log('invokeUIMethod', arguments, argStr, "*");
     ...
     * @returns {Boolean} context.isSelfHosted
     */
    function resolveContext() {
        var context = this.context;
        var stateMode = this.state.mode.toLowerCase();
    ...
    return {
        init: init,
        ApiRequestConstructor: ApiRequestConstructor,
        //invokeUIMethod: invokeUIMethod,
        REST: {
            call: restCall,
        ...
        Util: {
            md5: md5,
            encodeUtf82: encodeUTF8,
            encodeUtf8: encodeUtf8,
            decodeUtf82: decodeUTF8,
